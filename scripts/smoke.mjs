...
const first = await page.evaluate(() => ({
  ...
  selectionCards: document.querySelectorAll('.character-select-card').length,
  hud: Boolean(document.querySelector('.hud-shell')),
  game: Boolean(window.__RICH_GAME__)
}));
await page.locator('.character-select-card[data-player-id="3"]').dispatchEvent('click');
await page.locator('[data-action="confirm-character"]').dispatchEvent('click');
